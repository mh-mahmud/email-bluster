                    ...
                    <form class="m-form" @submit.prevent="updateCampaign">
                        <div class="m-portlet__body">
                            <div class="m-form__section m-form__section--first">
                                ...
                                <div class="form-group m-form__group row" :class="errors.has('email_limit') || validationErrors.email_limit ? 'has-error' : ''">
                                    <label class="col-lg-2 col-form-label"  for="email_limit">Per Hour Mail Limit:<span class="required">*</span></label>
                                    <div class="col-lg-6">
                                        <input data-vv-as="Email Limit" name="email_limit" v-validate="'required|max:255'"  v-model="campaign.email_limit" type="text" class="form-control m-input" placeholder="Email Limit">
                                        <span class="m-form__help" v-if="errors.has('email_limit') || validationErrors.email_limit">
                                            {{ errors.first('email_limit') || validationErrors.email_limit[0] }}
                                        </span>
                                    </div>  
                                </div>
...
export default {
  ...
  methods: {
    ...
    getCampaignById(id) {
        let vm = this;
        let refUrl = 'api/campaign/'+id+'/edit';
        if(typeof commonLib != 'undefined'){
            commonLib.blockUI({target: ".m-content",animate: true,overlayColor: 'none'});
        }
    ...
    deleteAttachedFile(fileId, index){
        let data = this.data.attachments;
        let url = 'api/campaign/delete-attachment/'+fileId;
        let msg = "Are you want to delete this file ?";
