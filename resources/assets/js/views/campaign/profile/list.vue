                                ...
                                <table class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline collapsed" id="m_table_1" role="grid" aria-describedby="m_table_1_info" style="width: 1044px;">
                                    ...
                                    <tbody>
                                        <tr v-for="(campaign, index) in data.data" v-bind:key="campaign.id">
                                            ...
                                            <td>
                                                <a v-if="campaign.invalid_emails > 0" :href="baseUrl+'api/campaign/export-invalid-email/'+campaign.id">{{campaign.invalid_emails}}</a>
                                                <span v-else>{{campaign.invalid_emails}}</span>
                                            </td>
                                            <td>{{campaign.send}}</td>
                                            <td>{{campaign.email_limit}}</td>
                                            <td>{{data.campStatus[campaign.status]}}</td>
                                            <td>
                                                <button v-if="campaign.status !='D'" type="button" class="btn btn-sm btn-success" @click.prevent="updateCampStatus(index, campaign.id,campaign.status);"> 
