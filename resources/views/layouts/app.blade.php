...
<body class="m-page--fluid m--skin- m-content--skin-light2 m-header--fixed m-header--fixed-mobile m-aside-left--enabled m-aside-left--skin-dark m-aside-left--offcanvas m-footer--push m-aside--offcanvas-default">
    ...
    <?php
        if(session(config('msg_label.RESPONSE_MSG'))){
            $fashMsg = session(config('msg_label.RESPONSE_MSG'));
    ?>
    <script type="text/javascript">
        commonLib.iniToastrNotification('<?php echo $fashMsg[config('msg_label.MSG_TYPE')]; ?>', '<?php echo $fashMsg[config('msg_label.MSG_TITLE')]; ?>', '<?php echo $fashMsg[config('msg_label.MSG_MESSAGE')]; ?>');
    </script>
    <?php
        }
    ?>
